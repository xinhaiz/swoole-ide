 ...
 */
function swoole_async_readfile(string $filename, callenable $callback){}
 ...
 * @param callenable $callback
 */
function swoole_async_writefile(string $filename, string $data, callenable $callback){}
 ...
 * @return boolean
 */
function swoole_async_read(string $filename, callenable $callback, int $size = 8192, int $offset = 0){}
 ...
 * @return boolean
 */
function swoole_async_write(string $filename, string $content, int $offset = -1, callenable $callback = NULL){}
 ...
 * @param callenable $callback
 */
function swoole_async_dns_lookup(string $domain, callenable $callback){}
 ...
 * @return boolean
 */
function swoole_event_add(int $sock, callenable $read_callback, callenable $write_callback = null, int $event_flag = null){}
 ...
 * @param int $flag
 */
function swoole_event_set($fd, callenable $read_callback, callenable $write_callback, int $flag){}
 ...
 * @param callenable $callback
 */
function swoole_timer_add(int $interval, callenable $callback){}
 ...
 */
function swoole_timer_tick(int $ms, callenable $callback, callenable $param = null){}
 ...
 */
function swoole_timer_after(int $after_time_ms, callenable $callback_function){}
...
final class swoole_server {
     ...
     * 
     * 别名：bool swoole_server_handler(swoole_server $serv, string $event_name, callenable $event_callback_function);
     *      第一个参数是swoole的资源对象
     * 
     * @param string $event_name
     * @param callenable $event_callback_function
     */
    public function handler(string $event_name, callenable $event_callback_function){}
     ...
     * @param callenable $callback_function 时间到期后所执行的函数，必须是可以调用的。callback函数不接受任何参数
     */
    public function after(int $after_time_ms, callenable $callback_function){}
...
final class swoole_process {
     ...
     * @return boolean
     */
    public static function signal(int $signo, callenable $callback){}
}
